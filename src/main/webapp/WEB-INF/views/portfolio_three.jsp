  ...
  <head>
   ...
   <!-- haerin  -->
    ...
    $(document).ready(function () {
		...
		$("#select_portfolio_id").val(portfolio_ID);
        $("#select_template_id").val(portfolio_Info[0].tem_id);
        $("#select_template_html").val(portfolio_Info[0].tem_name);
        var new_link =  "resources/images/" + portfolio_Info[0].tem_name +".png";
		$("#preview_portfolio").attr("src", new_link);
		$("#1page_myForm").attr("action", "portfolio_edit");

		$('input[name=1page_pName]').val(portfolio_Info[0].port_title);
		$('select[name=1page_public]').val(portfolio_Info[0].port_public);
		$('#select_portfolio_name').val(portfolio_Info[0].port_title);
		$('#select_portfolio_public').val(portfolio_Info[0].port_public);
         ...
         //수정의 경우 
        for(var e = 0; e < data_content.length;e++){
        	add_count++; //포트폴리오에 넣는 전체 항목 
            var data_num = data_content[e].data_no;
            ...
            if(data_content[e].item_id ==1){//type이 file 일때
                ...
                var realTag = $('<input type="file" name="file1" style="display: none;"><input type="hidden" name="photo_name" value="'+data_content[e].content[0]+'">');
                $($('#1page_count' + add_count).children()[0]).append(realTag);
                //$('input[name=file1]').val(data_content[e].content[0]);
                
                

             }else{

	            for(var f = 0; f < data_num; f++){
        ...
        //정보 저장 
          ...
          $("#1page_preview_button").click(function() {
    		  $("#1page_preview").load( '<%=request.getContextPath()%>/preview', {portfolio_preview: 'portfolio_preview' } );
    	  });
