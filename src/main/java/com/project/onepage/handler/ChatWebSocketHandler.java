package com.project.onepage.handler;
...
public class ChatWebSocketHandler extends TextWebSocketHandler implements InitializingBean{
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        ChatRoomVO chatRoom = chatService.selectChatRoom(Integer.toString(chatMessage.getRoomId()));
        System.out.println(chatMessage);
        System.out.println(chatRoom);
        // 채팅 세션 목록에 채팅방이 존재하지 않고, 처음 들어왔고, DB에서의 채팅방이 있을 때
        // 채팅방 생성
        if(RoomList.get(Integer.toString(chatRoom.getId())) == null && chatMessage.getMessage().equals("ENTER-CHAT") && chatRoom != null) {
            ...
            // sessionList에 추가
            sessionList.put(session, Integer.toString(chatRoom.getId()));
            // RoomList에 추가
            RoomList.put(Integer.toString(chatRoom.getId()), sessionTwo);
            // 확인용
            System.out.println("채팅방 생성");
        }
        
        // 채팅방이 존재 할 때
        else if(RoomList.get(Integer.toString(chatRoom.getId())) != null && chatMessage.getMessage().equals("ENTER-CHAT") && chatRoom != null) {
            
            // RoomList에서 해당 방번호를 가진 방이 있는지 확인.
            RoomList.get(Integer.toString(chatRoom.getId())).add(session);
            // sessionList에 추가
            sessionList.put(session, Integer.toString(chatRoom.getId()));
            // 확인용
        ...
        // 채팅 메세지 입력 시
            ...
            // 메세지에 이름, 이메일, 내용을 담는다.
            TextMessage textMessage = new TextMessage(chatMessage.getName() + "," + chatMessage.getUserId() + "," + chatMessage.getMessage());
            
            // 현재 session 수
            int sessionCount = 0;
 
            // 해당 채팅방의 session에 뿌려준다.
            for(WebSocketSession sess : RoomList.get(Integer.toString(chatRoom.getId()))) {
