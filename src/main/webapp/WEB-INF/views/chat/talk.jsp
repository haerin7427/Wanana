...
<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/resources/css/chat/talk.css?ver=2">
...
<body>
	...
	<div class="chatIcon font_jua">
		<img src="<%=request.getContextPath()%>/resources/images/chat-icon.png" class="iconImg">
	</div>
	...
	<div class="chatContainer display-none">
		<div class="chatTop">
			...
			<div class="floatRight">
				<img src="<%=request.getContextPath()%>/resources/images/chat-close.png" class="btnImg close">
			</div>
			<div class="floatRight">
				<img src="<%=request.getContextPath()%>/resources/images/chat-minus.png" class="btnImg down">
			</div>
		</div>
	...
	<div class="chatListContainer font_jua display-none">
		<div class="chatTop">
			<div style="padding: 10px; margin-left: 4px;">Wanana 채팅 리스트</div>
		</div>
		<div class="chatList">
			<!-- 동적 생성 -->
		</div>
    			...
    			success:function(data){
					...
					if(data.length > 0){
						...
						for(var i in data){
							...
							// 자신이 구매자 입장일 때
							if(data[i].senderId == "${sessionScope.ID}"){
								// 현재 판매자가 로그인 상태 일 때
								if(loginList.indexOf(data[i].receiverId) != -1){
									$div = $("<div class='chatList_box enterRoomList' onclick='enterRoom(this);'>").attr("roomId",data[i].id).attr("userId",data[i].receiverId);
								}
								// 현재 판매자가 로그아웃 상태 일 때
								else{
									$div = $("<div class='chatList_box2 enterRoomList' onclick='enterRoom(this);'>").attr("roomId",data[i].id).attr("userId",data[i].receiverId);
								}
								$img = $("<img class='profile_img'>").attr("src", "resources/images/default_userpic.png");
								$divs = $("<div class='userNameId'>").text(data[i].receiverName);
							}
							// 자신이 판매자 입장일 때
							else{						
								// 현재 구매자가 로그인 상태 일 때
								if(loginList.indexOf(data[i].senderId) != -1){
									$div = $("<div class='chatList_box enterRoomList' onclick='enterRoom(this);'>").attr("roomId",data[i].id).attr("userId",data[i].senderId);
								}
								// 현재 구매자가 로그아웃 상태 일 때
								else{
									$div = $("<div class='chatList_box2 enterRoomList' onclick='enterRoom(this);'>").attr("roomId",data[i].id).attr("userId",data[i].senderId);
								}								
								$img = $("<img class='profile_img'>").attr("src", "resources/images/default_userpic.png");
								$divs = $("<div class='userNameId'>").text(data[i].senderName);
							}
	...
	<script>
        ...
        function enterRoom(obj){
        	...
        	roomId = obj.getAttribute("roomId");
